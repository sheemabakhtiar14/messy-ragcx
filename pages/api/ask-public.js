  ...
  try {
    ...
    if (docError) {
      ...
      return res.status(500).json({ 
        ...
        answer: 'Sorry, I encountered an error while searching for documents. Please try again.'
      });
    }

    if (!documents || documents.length === 0) {
      return res.status(200).json({ 
        answer: 'I don\'t have any documents to search through yet. Please upload some documents first using the main application, then try asking questions.',
        sources: [],
        stats: {
          documentsSearched: 0,
          relevantDocuments: 0
    ...
    console.log(`Found ${documents.length} documents to search:`, documents.map(d => d.filename));
    ...
    if (relevantDocs.length === 0) {
      ...
      const broadResults = documents.filter(doc => {
        const content = (doc.content || '').toLowerCase();
    ...
    return res.status(200).json({
      ...
      demo_note: "This is a demo version. In the full system, you'd have access to your personal authenticated document library."
    });
