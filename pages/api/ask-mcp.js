  ...
  try {
    ...
    if (organization_id) {
      try {
        ...
        console.log(
          `MCP Ask - User ${userId} has access to organization ${organization_id}`
        );
      } catch (error) {
      ...
      if (search_scope === "all" || search_scope === "organization") {
        ...
        for (const membership of userOrgs.memberships || []) {
          if (
            ...
            const orgDocs = await queryUserDocuments(
              userId,
    ...
    if (totalAccessibleDocs === 0) {
      return res.json({
        answer:
          "You don't have access to any documents yet. Please upload a document first or join an organization to ask questions.",
        sources: [],
    ...
    try {
      ...
      console.log(
        `MCP Ask - Found ${searchResults.length} similar chunks for user ${userId}`
      );
    } catch (error) {
      console.error("MCP Ask - Error performing semantic search:", error);
      return res
        .status(500)
        .json({ error: "Error performing semantic search" });
    }

    if (searchResults.length === 0) {
      return res.json({
        answer:
          "I couldn't find any relevant information in your accessible documents to answer this question.",
        sources: [],
    ...
    console.log(
      "MCP Ask - Source breakdown:",
      contextWithScores.sourceBreakdown
    );
...
Answer:`;
    ...
    if (typeof answer === "string") {
      ...
      if (answer.length === 0) {
        answer =
          "I found relevant context but couldn't generate a proper answer. Please rephrase your question or try a different approach.";
      }
    } else {
      answer =
        "I couldn't generate a proper answer from the available context. Please try rephrasing your question.";
    }
